...
def login():
    if current_user.is_authenticated:
        from app.services.auth_service import NotificationManager
        NotificationManager.send_already_logged_in()
        return redirect(url_for('main_frontend.index'))

    form = LoginForm()
    if form.validate_on_submit():
        success = process_login(form.email.data, form.password.data)  
        if success:
            return redirect(url_for('main_frontend.index'))     
